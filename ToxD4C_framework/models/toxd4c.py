...
class ToxD4C(nn.Module):
    ...
    def enable_attention_storage(self, enable: bool = True):
        ...
        Args:
            enable: True to enable storage, False to disable
        """
    ...
    def get_all_attention_weights(self) -> Dict[str, torch.Tensor]:
        """
        Get cached attention weights from all attention layers.

        Returns:
            Dict: {layer_name: attention weight tensor [batch, heads, atoms, atoms]}
        """
        attention_dict = {}
        for name, module in self.named_modules():
    ...
    def clear_attention_cache(self):
        """Clear cache from all attention layers."""
        for name, module in self.named_modules():
    ...
    def visualize_attention(
        ...
        """
        Visualize attention for a single molecule.

        Args:
            data: Model input data
            smiles: SMILES string of the molecule
            layer_name: Name of attention layer to visualize (None uses first available)
            heads_to_show: List of attention head indices to display
            save_path: Path to save the visualization image
            sample_idx: Sample index in the batch

        Returns:
            Dict: Dictionary containing attention weights and distance matrix
        """
        ...
        if not attention_dict:
            print("Warning: No cached attention weights found")
            return None
        ...
        if layer_name and layer_name in attention_dict:
            ...
            # Use the first available attention layer
            layer_name = list(attention_dict.keys())[0]
            attention = attention_dict[layer_name]

        # Extract attention for a single sample
        if attention.dim() == 4:
        ...
        if coordinates is not None:
            ...
            # If batch data, extract corresponding sample
        ...
        # If save path provided, generate visualization
        if save_path:
            try:
                ...
                print("Please ensure attention_visualization.py is in current path")
