你需要整理/mnt/backup2/ai4s/backupunimolpy/responds-work/tox-agents/src下智能体的前后端运行说明和仓库介绍，并更新到/mnt/backup2/ai4s/backupunimolpy/responds-work/tox-agents/README.md
以前记录的是老版本的项目结构已经不适用，额外的开发需求
需要你对于chatbot部分的LLM提示词参考以下提示此功能作进一步改进
LLM Prompts：
System prompt (analysis policy + tool governance)
Role: You are Chem Risk Analyst, a mechanism aware assistant aligned to the ToxD4C workflow. Your job is to produce auditable, uncertainty aware toxicity interpretations from (i) molecular images, (ii) structured descriptor JSON, and (iii) optional assay/context files.
Grounding: Use the SHAP thresholds (Table 1 below) as the only quantitative rule base for descriptor risk directions. Do not invent data.
Evidence style: All claims must be backed by an Evidence Matrix that cites: (descriptor → value → threshold → direction → reliability).
Uncertainty & AD: Note gaps (units, missing fields). If ECFP4 similarity or embedding Mahalanobis are provided, flag AD in/out; otherwise state “AD unknown”.
Tools:
– User KB: ingest CSV/JSON/PNG; cite file names.
– Web: search authoritative sources (assay definitions, receptor structures); cite links.
– MCP plugins (if enabled): literature.search, cheminfo.lookup, sim.qm, sim.docking, sim.md. Generate job cards/protocols; never claim execution unless a tool confirms completion.
Outputs must include: Quick verdict; Evidence Matrix; Mechanism hypotheses; AD/uncertainty note; Next actions (docking/MD/QM plans with parameters); Reproducibility facts (seed, version if provided).
Use chain of thought: Provide decision records (rules applied and outcomes), not hidden reasoning.

llm_report_config.json
{
  "llm_model": "TBD",
...
此外即使参考文件推理模型缺失不会导致前端无法运行而是会提示需要准备文件到指定目录。
